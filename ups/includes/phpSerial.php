 ...
 *
 * @author Remy Sanchez <thenux@gmail.com>
 * @thanks Aurelien Derouineau for finding how to open serial ports with windows
...
class phpSerial
{
  ...
  public function deviceSet ($device)
  {
    if ($this->_dState !== SERIAL_DEVICE_OPENED) {
      if ($this->_os === "linux") {
        ...
        if (preg_match("@^COM(\d+):?$@i", $device, $matches) and $this->_exec(exec("mode " . $device . " xon=on BAUD=2400")) === 0) {
  ...
  public function serialflush ()
  {
    ...
    if (fwrite($this->_dHandle, $this->_buffer,1) !== false) {
      $this->_buffer = "";
